...
def center_of_mass(masses, positions):
    ...
    """
    saturn.speak("Calculating center of mass. All systems gravitate toward balance.")
    masses = np.array(masses)
    positions = np.array(positions)

    if len(masses) != len(positions):
        saturn.speak("Mass and position list mismatch detected. Gotta fix that first.")
        raise ValueError("Mass and position lists must be the same length.")

    total_mass = np.sum(masses)
    if total_mass == 0:
        saturn.speak("No mass? No motion. Try again with some substance.")
...
def total_momentum(masses, velocities):
    ...
    """
    saturn.speak("Summing up the momentum—motion in numbers.")
    masses = np.array(masses)
    velocities = np.array(velocities)

    if len(masses) != len(velocities):
        saturn.speak("Momentum mismatch: check those mass and velocity lists!")
...
def collision_momentum(m1, v1, m2, v2, is_elastic=True):
    ...
    """
    if is_elastic:
        saturn.speak("Elastic collision engaged! No energy lost, no regrets.")
        v1_final = ((m1 - m2) / (m1 + m2)) * v1 + ((2 * m2) / (m1 + m2)) * v2
        v2_final = ((2 * m1) / (m1 + m2)) * v1 + ((m2 - m1) / (m1 + m2)) * v2
        return v1_final, v2_final
    else:
        saturn.speak("Inelastic collision detected. These two are sticking together.")
...
def impulse_momentum(mass, v_initial, v_final):
    ...
    """
    saturn.speak("Impulse incoming—changing momentum like flipping a switch.")
